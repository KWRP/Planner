...
Event::Event(const char *filepath, std::string date, std::string title, std::string description,
    ...
    if (checkDate(day, month, year)) {
        bool q = addEvent(filepath, day, month, year, title.c_str(), description.c_str(),
                          time.c_str(), durations.c_str());
    } else {
        bool x = createDate(day, month, year);
        bool y = addEvent(filepath, day, month, year, title.c_str(), description.c_str(),
                          time.c_str(), durations.c_str());
    }
